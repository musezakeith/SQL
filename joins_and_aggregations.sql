...
-- 🔹 Aggregations with GROUP BY (Single Table)
...
-- 🔹 Multi-Table Aggregations (JOIN + GROUP BY)
...
-- (orders ➡️ accounts ➡️ sales_reps)
SELECT 
...
-- Above query followed the chain (orders ➡️ accounts ➡️ sales_reps)
...
-- 🔹 Conditional Aggregations (CASE + SUM)
/*
...
-- 🔹 Advanced Aggregation & Business Analysis
...
-- DISTINCT ensures you don’t double‑count the same record if it appears multiple times due to joins.
