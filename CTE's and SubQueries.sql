...
-- Keep only orders where order amount > average order amount
-- Orders total amount Vs Average order value 
SELECT *
FROM public.orders
WHERE total_amt_usd > (
...
-- compute avg of all order 
-- filter companies that spent more per order than average
-- compute avg total_amt_usd for those companies
